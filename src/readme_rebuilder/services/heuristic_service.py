...
_RE_CI_TREE = re.compile(r'gitlab-ci|github/workflows', re.I)
...
class HeuristicsService:
    ...
    def derive_project_facts(
        ...
        has_tests = False
        has_ci = False
        has_git_repo = bool(git_context.get('has_git_repo')) if git_context else (project_path / '.git').exists()
